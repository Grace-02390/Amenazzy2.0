...
public class AdminProductoController {
    ...
    public String gestionarProductos(Model model) {
        ...
        return "admin_productos"; // Esto buscará src/main/resources/templates/admin_productos.html
    }
    ...
    public ResponseEntity<Map<String, Object>> guardarProducto(@RequestBody Producto producto) {
        ...
        try {
            // Asegurarse de que los valores de los ENUMs se conviertan correctamente
            // El JSON puede enviar "hombre" pero el Enum es GeneroProducto.hombre
            if (producto.getGenero() != null) {
                // Convertir el String del JSON a la instancia del Enum
                producto.setGenero(Producto.GeneroProducto.valueOf(producto.getGenero().name().toLowerCase())); // CORREGIDO
            }
            if (producto.getTalla() != null) {
                // Convertir el String del JSON a la instancia del Enum
                producto.setTalla(Producto.TallaProducto.valueOf(producto.getTalla().name().toUpperCase())); // CORREGIDO
            }
