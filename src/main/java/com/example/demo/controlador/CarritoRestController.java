...
public class CarritoRestController {
    ...
    public ResponseEntity<Map<String, Object>> actualizarCantidadEnCarrito(@RequestBody UpdateCartItemRequest request, HttpSession session) {
        ...
        try {
            Optional<com.example.demo.modelo.Carrito> updatedItem = carritoService.actualizarCantidadEnCarrito(request.getItemId(), request.getCantidad());

            if (updatedItem.isPresent()) {
                // Recalcular totales del carrito
                List<com.example.demo.modelo.Carrito> carritoItems = carritoService.findByUsuario(usuarioAutenticado);
                BigDecimal subtotal = BigDecimal.ZERO;
                BigDecimal totalDescuento = BigDecimal.ZERO;

                for (com.example.demo.modelo.Carrito item : carritoItems) {
